...
namespace Isles.Pipeline
{
    ...
    public class SkinnedModelProcessor : ModelProcessor
    {
                                             ...
                                             ContentProcessorContext context)
        {
            ...
            //BoneContent skeleton = FindSkeleton(input);
            ...
            //NodeContent skeleton;
            //AnimationContentDictionary animations = FindAnimations(input);
            //IList<NodeContent> bones = FindAndFlattenSkeleton(input, animations, out skeleton);
            ...
            //animationClips = ProcessAnimations(FindAnimation(input), bones);
        ...
        /// <summary>
        /// A replacement for MeshHelper.Find Skeleton
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
        static IList<NodeContent> FindAndFlattenSkeleton(
            NodeContent input,
            AnimationContentDictionary animations,
            out NodeContent skeleton)
        {
            // Find whatever node that the animation affects,
            // then create a hierarchy containing those nodes

            // For now, just flatten everything :)
            IList<NodeContent> nodes = new List<NodeContent>();

            // Don't add skeleton
            FlattenSkeleton(input, ref nodes);
        ...
        static void FlattenSkeleton(NodeContent input, ref IList<NodeContent> nodes)
        {
            if (input != null)
            {
                nodes.Add(input);
                ...
                foreach (NodeContent node in input.Children)
                    FlattenSkeleton(node, ref nodes);
            }
        }

        /// <summary>
        /// Find the first animation from a node hierarchy
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
        static AnimationContentDictionary FindAnimations(NodeContent input)
        {
            if (input != null && input.Animations.Count != 0)
            ...
            foreach (NodeContent node in input.Children)
            {
                animations = FindAnimations(node);
        ...
        /// </summary>
        static void FlattenTransforms(NodeContent node, NodeContent skeleton)
        {
            foreach (NodeContent child in node.Children)
            {
                // Don't process the skeleton, because that is special.
                if (child == skeleton)
                //if (child.Name.Equals("IK Chain01"))
                    continue;
